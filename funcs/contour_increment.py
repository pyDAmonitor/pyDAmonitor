...
def contour_increment(datasets, parms, fig_name=None):
    ...
    half = plot_box_width / 2.0
    left = cen_lon - half
    right = cen_lon + half
    half = plot_box_height / 2.0
    bot = cen_lat - half
    top = cen_lat + half
    ...
    gl1.xlocator = mticker.FixedLocator(np.arange(-180.0, 181.0, 5.0))
    gl1.ylocator = mticker.FixedLocator(np.arange(-80.0, 91.0, 5.0))
